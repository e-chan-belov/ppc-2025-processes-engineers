...
std::vector<int> BatcherLeftMerge(std::vector<int> &left_arr_, std::vector<int> &right_arr_, int local_arr_size, int rank, MPI_Comm comm) {
    ...
    MPI_Sendrecv(even_arr.data(), static_cast<int>(even_arr.size()), MPI_INT, rank + 1, 0,
        odd_arr.data(), static_cast<int>(odd_arr.size()), MPI_INT, rank + 1, 0,
...
std::vector<int> BatcherRightMerge(std::vector<int> &left_arr_, std::vector<int> &right_arr_, int local_arr_size, int rank, MPI_Comm comm) {
    ...
    MPI_Sendrecv(odd_arr.data(), static_cast<int>(odd_arr.size()), MPI_INT, rank - 1, 0,
        even_arr.data(), static_cast<int>(even_arr.size()), MPI_INT, rank - 1, 0,
        comm, MPI_STATUS_IGNORE);
...
void LeftProcAct(int rank, std::vector<int> &local_arr, int local_arr_size, MPI_Comm comm) {
    ...
    MPI_Sendrecv(local_arr.data(), local_arr_size, MPI_INT, rank + 1, 0,
        right_arr.data(), local_arr_size, MPI_INT, rank + 1, 0,
        comm, MPI_STATUS_IGNORE);
...
void RightProcAct(int rank, std::vector<int> &local_arr, int local_arr_size, MPI_Comm comm) {
    ...
    MPI_Sendrecv(local_arr.data(), local_arr_size, MPI_INT, rank - 1, 0,
        left_arr.data(), local_arr_size, MPI_INT, rank - 1, 0,
        comm, MPI_STATUS_IGNORE);
...
bool BelovEShellBatcherMPI::RunImpl() {
    ...
    MPI_Scatter(arr.data(), local_arr_size, MPI_INT,
    ...
    MPI_Allgather(local_arr.data(), local_arr_size, MPI_INT,
        arr.data(), local_arr_size, MPI_INT,
        MPI_COMM_WORLD);
    arr.resize(n);
    GetOutput() = arr;
    return true;
